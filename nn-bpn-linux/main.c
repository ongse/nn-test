...
/******************************************************************************
NN Application Specific Training Data
******************************************************************************/
...
/******************************************************************************
Application Initialization and Finalization
******************************************************************************/
void InitializeApplication(NET* Net)
{
	...
	/* application specific */
	NormalizeTrainingData(&Sunspots, &Sunspots_, NUM_YEARS, Mean);
...
/******************************************************************************
Application Specific NN Test and Evaluate
******************************************************************************/
...
int main(int argc, char *argv[])
{
	...
	do {
		TrainNet(&Net, 10);		//train network
		TestNet(&Net);			//test network
		if (TestError < MinTestError)				//error is still continue to reduce
		{
			MinTestError = TestError;					// testError becoming smaller
			SaveWeights(&Net);							// save new improved weights
			fprintf(f, " - saving Weights ...");
		}
		else if (TestError > 1.2 * MinTestError)	//stop if error change to become bigger
		{
			Stop = TRUE;
			RestoreWeights(&Net);
