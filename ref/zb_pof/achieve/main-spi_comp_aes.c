...
char core_ft, core_sl, core_width, core_seg, core_1st, second_dword, second_dword_sdm_comp, core_switch, core_switch_sdm_compress, switch_no;
char comp_high_nibble, comp_32_nibble, start_comp_output_odd;
...
{
	printf("\n    ------------------------------------------------------------------------");
	printf("\n    Altera Image Generation Software Tool v4.1");
	...
	while(1)
	{
		...
		else if(findString("VERSION", singleSmlBuf_ptr)!=NOTFOUND)
		{
			...
			if(compareString("4.1", extractPara_ptr)==SAME)
		...
		else if(findString("AES_POF_CREATE", singleSmlBuf_ptr)!=NOTFOUND)
		{
			create_aes_pof();
		}
	}
...
void iocsr_data_output(void)
{
	...
	for(i=0;i<4;i++)					//start - SDM AES portion
	{
		fputc('0', sdm_aes_core_outputdat);
		fputc('0', sdm_aes_core_outputdat);
		if(i == 0)
			fprintf(sdm_aes_core_outputdat, "  //iocsr frame start (start dword) %i", iocsr_frame_no+1);
	...
	for(i=0;i<(((ioscr_length+ioscr_pad)*4)+4);i++)		
	{
		...
		fputc(j, sdm_aes_core_outputdat);		
		j = hextxt[1];
		fputc(j, spi_aes_core_outputdat);
		fputc(j, spi_aes_comp_core_outputdat);
		fputc(j, sdm_aes_core_outputdat);
...
void core_data_generation(void)	//*** Core data generation
{
	...
	*core_data_ptr = j;
	...
	printf("\n    random number = 0x%x", (flipU16(random_num) & 0xffff));
...
{	
	...
	if(core_1st==1)									//core data - SPI AES Compressionportion
	{
		for(i=0;i<(start_filler_no*4);i++)
		{
...
void core_din_output(void)
{
	...
	else if(core_width==32)
	{
		for(i=0;i<((core_length*4)+8);i++)		
		{
			...
			if((i==(((core_length*4)+8)-4))&&(core_seg == 32)&&(core_1st==1))	//ARIN
				fputc('1', corecffdat);
			else
				fputc('0', corecffdat);
				
			if((i==(((core_length*4)+8)-4))&&(core_seg == 32))		//ARCE & DRBCE
				fprintf(corecffdat, "11");
			else
				fprintf(corecffdat, "00");
...
void create_aes_pof(void)
{
	...
	for(i=0;i<aes_core_data_32bit_cnt;i++)				//*** 128-bit per block (per loop) - SPI AES portion
	{
		...
		//debugmsg("\niv = ");
		//for(j=0;j<16;j++)
		//	debugmsg("%x ",aes_iv[j] & 0x00ffffffff);
		...
		//debugmsg("\nenc_counter = ");
		//for(j=0;j<16;j++)
		//	debugmsg("%x ",enc_counter[j] & 0x00ff);
		//debugmsg("\nenc_counter_32bit = ");
		//for(j=0;j<4;j++)
		//	debugmsg("%x ",enc_counter_32bit[j] & 0x00ffffffff);
		...
		//debugmsg("\ndata, enc_data, iv, enc_out = %x %x ", aes_core_data[pof_count+0], aes_encrypted_data[pof_count+0]);
		//for(j=0;j<16;j++)
		//	debugmsg("%x", aes_iv[j] & 0xff);
		//debugmsg(" ");
		//for(j=0;j<4;j++)
		//	debugmsg("%x", enc_counter_32bit[j] & 0xffffffff);
		//debugmsg("\ndata, enc_data = %x %x ", aes_core_data[pof_count+1], aes_encrypted_data[pof_count+1]);
		//debugmsg("\ndata, enc_data = %x %x ", aes_core_data[pof_count+2], aes_encrypted_data[pof_count+2]);
		//debugmsg("\ndata, enc_data = %x %x ", aes_core_data[pof_count+3], aes_encrypted_data[pof_count+3]);		
		pof_count += 4;
	...
	for(i=0;i<(core_comp_length/16);i++)				//*** 128-bit per block (per loop) - SPI AES Compression
	{
		...
		for(j=0;j<16;j++)
		{
			aes_comp_encrypted_data[pof_count] = core_aes_comp_data[pof_count] ^ enc_counter[j];
			//debugmsg("%x ", aes_comp_encrypted_data[pof_count] & 0xff);
			pof_count++;
		}
